const g=`Product	#SN.	Image	Code	Name	Category	Sell	Stock
...
`;function M(R){const r=R.split(/\r?\n/).map(o=>o.trim()).filter(Boolean);let P=0;for(let o=0;o<r.length;o++){const n=r[o].toLowerCase();if(n.includes("code")&&n.includes("name")){P=o+1;break}}const D=[];for(let o=P;o<r.length;o++){const n=r[o];let e=n.split("	").map(i=>i.trim()).filter(()=>!0);e.length===1&&(e=n.split(/\s{2,}/).map(i=>i.trim()));let t,a,c,s,l;if(e.length>=7)t=e[2]||"",a=e[3]||"",c=e[4]||"",s=e[5]||"",l=e[6]||"";else if(e.length===6)t=e[1]||"",a=e[2]||"",c=e[3]||"",s=e[4]||"",l=e[5]||"";else{const i=n.split(/\t|\s{2,}/).map(m=>m.trim()).filter(Boolean);if(i.length>=5)t=i[1],a=i.slice(2,i.length-3+2).join(" "),c=i[i.length-3],s=i[i.length-2],l=i[i.length-1];else continue}const u=parseFloat(String(s).replace(/[,\s]/g,""))||0;let d=parseInt(String(l).replace(/[^-0-9]/g,""),10);Number.isNaN(d)&&(d=0),D.push({code:t||"",name:a||"",category:c||"Others",price:u,stock:d})}return D.map((o,n)=>({id:n+1,...o}))}const S=M(g),C={Food:"🍖",Accessories:"🧸",Medicine:"💊",Service:"🛎️","Diagnostic Test":"🧪",Surgery:"🔪",Others:"📦"};export{C as categoryEmoji,S as default,S as products};
